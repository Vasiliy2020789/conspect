...
//=======Передаtм вкладки в основной массив данных
...
document.addEventListener("click", function name(event) {
	...
	if (event.target.closest('.tab')) {
		...
		for (i = 0; i < arrLessonTemaTabs.length; i++) {
			for (k = 0; k < arrLessonTemaTabs[i][5].length; k++) {
				...
				if (event.target === arrLessonTemaTabs[i][5][k]) {
					...
					//НУЖНО СЮДА ДОБАВИТЬ ЦИКЛ ПЕРЕБОРА КАК=========================================================================================================
					//_______Выводим выбраный пример НО ПЕРЕБИРАТЬ ВКЛАДКИ И ВЫПОЛНЯТЬ "getActive();"
					//ТОЛЬКО В УРОКЕ В КОТОРОМ НАЖАТА КНОПКА
		...
		//console.log(`Завершился цикл изменения активной вкладки. ОМД = ${arrLessonTemaTabs}`);

	}
	/*
//_______Выводим выбраный пример
for (i = 0; i < arrLessonTemaTabs.length; i++) {
//console.log(` i = ${i}`);//ПРОВЕРКА
for (k = 0; k < arrLessonTemaTabs[i][5].length; k++) {
	//console.log(` i = ${i},  k = ${k}`);//ПРОВЕРКА
	//console.log(`arrLessonTemaTabs[i][5].length = ${arrLessonTemaTabs[i][5].length}`);//ПРОВЕРКА
	//console.log(functionArr[i][k](canvasArr[i]));//ПРОВЕРКА
	if (arrLessonTemaTabs[i][4] === k) {
		console.log(`Выводим выбраный пример`);
		functionArr[i][k](canvasArr[i]);
		break;
	}
}
